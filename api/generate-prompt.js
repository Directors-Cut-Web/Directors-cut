...
const createSystemInstruction = (targetModel) => {
  switch (targetModel) {
    ...
    case 'Midjourney Video':
        return `You are a 'Midjourney-Maestro', an expert in crafting prompts for Midjourney Video. Your task is to take the user's inputs and assemble them into a single, perfectly formatted Midjourney prompt. The prompt must start with the descriptive text, combining Scene, Character, Camera, Lighting, Genre, and Format. After the text, append all of the following parameters, each separated by a space and a double-hyphen: --ar, --motion, --stylize, --chaos. If 'Style Raw' is enabled, add '--raw'. If a 'Negative Prompt' exists, add it using '--no'. If 'cref' or 'sref' URLs exist, add them. For example: 'A dragon breathes fire --ar 16:9 --motion high --stylize 250 --chaos 10 --raw --cref URL --sref URL --no smoke'.`;
  ...
  try {
    ...
    // --- MODIFICATION: Added new logic for Midjourney Video ---
        ...
        userPrompt = `
            Scene/Environment: ${inputs.scenePrompt || 'Not specified'}
            Character/Action: ${inputs.characterPrompt || 'Not specified'}
            Camera Settings: ${inputs.cameraSettingsPrompt || 'Not specified'}
            Lighting: ${inputs.lightingPrompt || 'Not specified'}
            Genre/Mood: ${inputs.genrePrompt || 'Not specified'}
            Format/Technical: ${inputs.formatPrompt || 'Not specified'}
            --ar: ${inputs.aspectRatio || '16:9'}
            --motion: ${inputs.motionLevel === 'High Motion' ? 'high' : 'low'}
            --stylize: ${inputs.stylize || 100}
            --chaos: ${inputs.chaos || 0}
            --raw: ${inputs.styleRaw ? 'Yes' : 'No'}
            --cref: ${inputs.crefUrl || 'None'}
            --sref: ${inputs.srefUrl || 'None'}
            --no: ${inputs.negativePrompt || 'None'}
        `;
